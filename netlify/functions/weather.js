// Netlify serverless function — proxies Buienradar's free public weather feed.
...
exports.handler = async function (event) {
  ...
  const lat = parseFloat(event.queryStringParameters?.lat) || 52.0907;
  const lon = parseFloat(event.queryStringParameters?.lon) || 5.1214;
  ...
  try {
    ...
    return { statusCode: 502, headers, body: JSON.stringify({ error: 'Weather service unavailable' }) };
  }
};
